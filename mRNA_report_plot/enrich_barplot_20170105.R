...
enrich_barplot <- function(enrich_merge_data,x_lab = "",y_lab = "Number of genes",break_label1,break_label2){
  p <- ggplot(enrich_merge_data,aes(y=Input_number,x=Term,fill=color))+
    ...
    scale_fill_manual(values = c('up'='#FF0000','down'='#00A08A','all'='#5BBCD6','expected'='black'),
                      breaks = c('expected','up','down','all'),labels = c('Expected no. of genes',paste("No.",break_label1," up-regulated genes",sep = " "),
                                                                          paste("No.",break_label2," up-regulated genes",sep = " "),
                                                                          'No. of all diff-expressed genes'))+
