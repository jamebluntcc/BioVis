...
cwd <- getwd()
diff_file <- 'N42_vs_N7.edgeR.DE_results.txt'
name <- 'C-b_vs_F-a'
out_dir <- cwd
p <- 0.05
lgfc <- 1
...
diff_file_info <- read.delim(diff_file)
dpa_results <- diff_file_info[,c("logFC","FDR")]
...
red_count <- count$Freq[which(count$Var1 == "red")]
green_count <- count$Freq[which(count$Var1 == "green")]
all_count_number <- red_count+green_count
#all_count <- paste("Genes",all_count_number,sep = ":")
...
red_label <- paste("Up",red_count,sep = ":")
green_label <- paste("Down",green_count,sep = ":")
#brewer for color
...
merge_plot <- cowplot::plot_grid(p,p,p,p,p,p,p,p,p,labels=LETTERS[1:9], ncol = 3, nrow = 3)
ggsave(filename=paste(out_dir,"/",name,".Volcano_plot.png",sep=""),type="cairo-png",plot=merge_plot,width = 12,height = 12)
